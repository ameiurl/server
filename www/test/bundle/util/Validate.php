...
class Validate extends \think\Validate
{
    ...
    //Mobile
    private static $Mobile = '/^(\+?\d{2})?((\d{2,3})|(\d{3}\-))?1(3|5|8)\d{9}$/';
    //Phone
    ...
    protected $regex = [
        'mobile' => '/^(\+?\d{2})?((\d{2,3})|(\d{3}\-))?1(3|5|8)\d{9}$/',
    ];
    ...
    public static function checkFormToken()
    {
        ...
        if($req->isPost())
        {
            $data = $req->param();
            $validate = new self();
            if(!$validate->check($data,['__token__' => 'token']))
            {
                throw new \RuntimeException("表单数据无效");
            }
        }
    }
     ...
     * @return boolean
     */
    public static function is_xss($text)
    {
        if (preg_match(self::$xss, trim($text))) {
            return true;
        }
        return false;
    }
